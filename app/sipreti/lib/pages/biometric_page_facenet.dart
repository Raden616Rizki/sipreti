...
class BiometricFacenetPage extends StatefulWidget {
  const BiometricFacenetPage({super.key});

  @override
  State<BiometricFacenetPage> createState() => _BiometricFacenetPageState();
}

class _BiometricFacenetPageState extends State<BiometricFacenetPage> {
  ...
  Future<void> _loadModel() async {
    try {
      _interpreter = await Interpreter.fromAsset('model/facenet.tflite');
    } catch (e) {
      if (mounted) {
  ...
  Future<List<double>> _getFaceEmbeddings(img.Image faceImage) async {
    ...
    var reshapedInput = input.buffer.asFloat32List().reshape([1, 160, 160, 3]);
  ...
  Future<Float32List> _loadAndNormalizeImage(img.Image image) async {
    final resizedImage = img.copyResize(image,
        width: 160, height: 160, interpolation: img.Interpolation.linear);

    final pixels = resizedImage.getBytes(format: img.Format.rgb);
    final Float32List floatPixels = Float32List(160 * 160 * 3);
