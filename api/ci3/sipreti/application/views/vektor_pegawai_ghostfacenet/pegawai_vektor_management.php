...
<body>
	<div class="navbar">
		...
		<div class="navbar-right">
			...
			<a class="active" href="/sipreti/vektor_pegawai_ghostfacenet">Biometrik Pegawai</a>
		</div>
	</div>
	...
	<div class="card-container">
		<div class="card">
						...
						<?php if (!empty($biometrik[$i])): ?>
							<div class="image-wrapper">
								...
								$foto_path = !empty($biometrik[$i]->url_foto)
									? base_url('uploads/vektor_pegawai_ghostfacenet/' . $biometrik[$i]->id_pegawai . '/' . $biometrik[$i]->url_foto)
								...
								<button class="delete-btn"
									onclick="openModal('<?= site_url('vektor_pegawai_ghostfacenet/delete_from_pegawai/' . $biometrik[$i]->id_pegawai . '/' . $biometrik[$i]->id_vektor_pegawai); ?>')">
			...
			<div class="form-actions">
				<a href="<?php echo site_url('vektor_pegawai_ghostfacenet/list_pegawai') ?>" class="btn-back">Kembali</a>
			</div>
		</div>
	</div>
	<footer class="footer">
...
<script>
	...
	document.getElementById('uploadInput').addEventListener('change', async function (event) {
		...
		for (let i = 0; i < files.length; i++) {
			...
			try {
				const response = await fetch('http://127.0.0.1:8000/attendance/face-register-ghostfacenet/', {
					method: 'POST',
					body: formData
				});
		...
		if (!errorOccurred) {
			window.location.href = "<?= site_url('vektor_pegawai_ghostfacenet/read_vektor_pegawai/' . $pegawai->id_pegawai) ?>";
		}
	});
