...
<body>
	...
	<div class="content-container">
		...
		<div class="toolbar">
			<form action="<?php echo site_url('vektor_pegawai_ghostfacenet/list_pegawai'); ?>" method="get" class="search-form">
				<input type="text" name="q" class="search-input" placeholder="Masukkan nama pegawai..."
					value="<?php echo $q; ?>">
				<button type="submit" class="search-btn">Cari</button>
				<?php if ($q <> ''): ?>
					<a href="<?php echo site_url('vektor_pegawai_ghostfacenet/list_pegawai'); ?>" class="reset-btn">Reset</a>
				<?php endif; ?>
			</form>
			<!-- <div>
				<form method="get" action="<?php echo site_url('vektor_pegawai_ghostfacenet/export_csv'); ?>"
				...
				<?php foreach ($pegawai_data as $pegawai): ?>
					<tr>
						...
						<td class="action-buttons">
							<a href="<?php echo site_url('vektor_pegawai_ghostfacenet/read_vektor_pegawai/' . $pegawai->id_pegawai); ?>"
		...
		fetch(`http://127.0.0.1:8000/attendance/upload-csv/?task_id=${taskId}`, {
			...
			.then(result => {
				if (result.message) {
					...
					window.location.href = "<?php echo site_url('vektor_pegawai_ghostfacenet/list_pegawai'); ?>";
				} else {
