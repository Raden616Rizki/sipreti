...
class Pegawai_model extends CI_Model
{
	...
	// get data by id api facenet
	public function get_by_id_facenet_api($id)
	{
		$this->db->select('
        pegawai.*, 
        jabatan.nama_jabatan,
    ...
    ');
		...
		if ($pegawai) {
			$this->db->select('face_embeddings');
			$this->db->from('vektor_pegawai_facenet');
			$this->db->where('id_pegawai', $id);
			$this->db->where('deleted_at IS NULL');
			$query = $this->db->get();
			$embeddings = [];
	...
	public function get_limit_data_facenet($limit, $start = 0, $q = NULL, $onlyActive = FALSE)
	{
		$this->db->select('
        pegawai.*, 
        jabatan.nama_jabatan, 
	...
	public function delete_by_nip_facenet($nip)
	{
		$pegawai = $this->db->get_where('pegawai', ['nip' => $nip])->row();
